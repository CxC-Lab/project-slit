...
### 2026-09-13: 지형에 첫 실제 텍스처가 들어갔습니다

Avatar Lake 지형 안쪽(inner)에 실제 암반 텍스처를 입혔습니다. 여러 후보를 실제 게임 화면에서 비교해, 원본 전체를 120x120으로 줄인 `E0_full_120`을 채택했습니다. 암반 크기는 캐릭터에 어울리고 반복 이음선은 드러나지 않습니다.

지금 개발 화면은 중간 상태입니다. 지형 안쪽만 실제 텍스처이고, 윗면과 옆면, 모서리 같은 나머지 아홉 가지 역할은 아직 색으로 구분한 진단 표현입니다.

미술 원본에서 게임 에셋까지는 매니페스트로 기록된 파이프라인을 거칩니다. 어떤 원본을 어떻게 잘라 줄였는지와 해시가 남아 있어 같은 결과를 다시 만들 수 있고, 승인된 에셋만 게임이 읽는 위치로 내보냅니다.
...
```
.\tools\run_avatar_lake.bat         현재 승인된 Avatar Lake 개발 화면
.\tools\run_avatar_lake_debug.bat   역할별 색상 진단 화면
```

`run_avatar_lake.bat`는 지금 승인된 표현을 보여 줍니다. 앞으로 새 역할의 텍스처가 승인되어도 같은 스크립트를 쓰면 됩니다. 진단 화면은 지형의 윗면, 아랫면, 옆면, 모서리를 색으로 구분해 배치를 확인할 때 씁니다.

후보를 하나씩 비교하거나 문제를 찾을 때만 타일셋을 직접 지정합니다.

```
.\build\Release\project_slit.exe avatar_lake --tileset avatar_lake_inner_trial_e0_120
```
...
`F12`를 누르면 현재 화면 PNG와 좌표 JSON이 `build/preview/captured_XXXX.*`에 함께 저장됩니다. JSON에는 지역과 실제로 쓴 타일셋, 지형 텍스처 파일도 기록되어 어느 화면이었는지 나중에 구분할 수 있습니다.
