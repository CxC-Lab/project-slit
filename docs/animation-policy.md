...
| WallSliding | 4 | Adopted |
| SlowFalling | 2 | Adopted |
| FastFalling | 2 | Adopted |
| Land | 4 | Adopted |

`Idle = 4`, `Walking = 4`, `Sprinting = 4` 는 proto_one 에서 실제로 채택해 런타임에서 재생 중인 값이다. Walking 과 Sprinting 은 제공된 원본 프레임이 각각 정확히 4장이었고 앞의 "프레임 수 후보" 표의 범위와도 맞아 채택했다. `Turn = 3`은 2026-09-10, proto_one에 제공된 raw Turn 프레임이 정확히 3장이라 채택했다. 이 값은 앞의 "프레임 수 후보" 표에 적힌 Turn의 범위(약 1~2)보다 많은데, 그 표는 "구속력 없음. 검토 중 바뀔 수 있다"고 명시된 참고용 후보 범위이지 강제 값이 아니므로, 실제 제공된 자산 기준으로 3을 채택했다. Turn은 코드의 `MovementState`에 대응하는 상태가 없다. 이는 미구현이라서가 아니라 의도한 설계다. 자세한 내용은 아래 "이동 상태가 아닌 애니메이션"을 참고한다. `Jumping = 3`은 2026-09-10, proto_one에 제공된 raw Jumping 프레임이 정확히 3장이라 채택했다. 코드의 `Jumping` MovementState는 이 문서 용어의 Jump Start(약 1)와 Airborne(약 1~2)를 함께 아우르므로, 두 후보 범위를 합친 상한(약 3)과 일치한다. `Falling = 3`은 같은 날, proto_one에 제공된 raw Falling 프레임이 정확히 3장이라 채택했다. 코드의 `Falling` MovementState는 이 문서 용어의 Airborne(약 1~2)에 대응하는데, 제공량 3은 그 범위보다 많다 — 다만 후보 범위 표는 "구속력 없음"이라 명시되어 있으므로 실제 제공된 자산 기준으로 3을 채택했다. `FastFalling = 2`는 같은 날, proto_one에 제공된 raw FastFalling 프레임이 정확히 2장이고 앞의 "프레임 수 후보" 표의 Fast Fall / Dive 범위(약 1~2)와도 정확히 맞아 채택했다. 2026-09-11에 raw FastFalling 원본이 4장으로 늘었다(가로세로비 실측상 앞 2장은 약 1.01~1.03, 뒤 2장은 약 1.15~1.21로 뒤 2장은 몸체 실루엣 자체가 눈물방울형으로 재구성되는 별개 카테고리). 이후 사용자가 의도를 명확히 했다 — FastFalling은 원래부터 "더 극단적인 낙하"를 표현하는 별도 상태이고, 뒤 2장(fast_falling03/04)은 실제 플레이에서 약하게 느껴졌던 앞 2장(01/02)의 대체 후보로 제작된 것이었다. 이에 따라 계약값(2)은 유지하되 실제 채택 원본을 01+02에서 03+04로 교체했다. 자세한 판단 근거는 `proto_one.manifest.json`의 FastFalling 항목과 상단 notes에 기록했다. `AirDashing = 3`은 2026-09-11, proto_one에 제공된 raw AirDashing 프레임이 정확히 3장이라 채택했다. 앞의 "프레임 수 후보" 표에 적힌 Air Dash 범위(약 2)보다 많은데, 그 표는 구속력이 없으므로 실제 제공된 자산 기준으로 3을 채택했다. `SlowFalling = 2`는 같은 날, proto_one에 제공된 raw SlowFalling 프레임이 정확히 2장이고 앞의 "프레임 수 후보" 표의 Hover 범위(약 1~2)와도 정확히 맞아 채택했다. `Land = 4`는 2026-09-11, proto_one에 제공된 raw Landing 프레임이 정확히 4장이라 채택했다. 앞의 "프레임 수 후보" 표에 적힌 Land 범위(약 1)보다 훨씬 많은데, 채택 전 4장을 육안 및 실측(알파 기준 body bbox, glow 제외)으로 검토한 결과 단순한 정지 포즈 변형이나 이펙트 전용 프레임이 아니라 실제 압축-회복 궤적이었다 — 프레임 0→1로 갈수록 body 가로세로비(H/W)가 0.76→0.64로 더 납작하게 눌렸다가(착지 충격의 최대 압축), 프레임 2→3에서 1.00→0.97로 원래의 둥근 형태에 가깝게 되돌아온다. Turn의 spin-blur 프레임처럼 카테고리가 다른 프레임이 섞여 있지도 않고, Jumping의 pod-tuck처럼 실루엣 일부만 다른 것도 아니라 몸체 전체가 매 프레임 다른 압축 단계를 나타내므로, 후보 범위 표가 구속력 없다는 원칙에 따라 제공된 4장 전부를 채택했다. `WallSliding = 4`는 같은 날, proto_one에 제공된 raw WallSliding 프레임이 정확히 4장이라 채택했다. 앞의 "프레임 수 후보" 표에 적힌 Wall / Hang 범위(약 1~3)보다 많은데, 채택 전 실측한 결과 전체 알파 bbox의 X 범위가 4장 모두 완전히 동일했고(X[234,1059]) Y 범위도 196~212 사이로 거의 고정되어 있어 캐릭터 위치/포즈 자체는 사실상 고정이고 왼쪽 추력 이펙트의 길이/형태만 프레임마다 미세하게 달라지는 구성임을 확인했다 — Turn처럼 카테고리가 섞인 것도, Landing처럼 실제 포즈가 진행되는 것도 아니라 Idle의 보정 있는 정지 루프에 더 가까운 성격이라, 제공된 4장 전부를 채택했다. 코드의 `MovementState`에 이미 `WallSliding` 값이 존재하므로(Turn/Land와 달리 실제 이동 상태), 벽에 붙어 있는 동안 유지되는 상태로 보고 loop=true로 다루었다. 이 문서의 상태 목록은 Land 로, 매니페스트와 코드는 Landing 으로 부른다. 같은 것이며 계약표에는 매니페스트 이름인 Landing 으로 적는다. 열두 값 모두 proto_one 만의 값이 아니라 앞으로 만들 모든 캐릭터가 따라야 하는 공통 프레임 수다. 나머지는 아직 정하지 않았으며 근거 없이 숫자를 채워 넣지 않는다.
...
Landing 이 두 번째 사례다. 착지 충격이 있을 때 잠깐 재생되고 끝난다. Turn 과 마찬가지로 `MovementState` 에 값을 추가하지 않고 현재 상태의 애니메이션을 잠시 덮어쓴다.

- 재생 때문에 이동이나 입력을 잠그지 않는다. 착지하자마자 걷거나 점프할 수 있다.
- 다른 이동 애니메이션이 필요해지면 즉시 취소한다. 그래서 이동 중 착지하면 한 프레임도 보이지 않을 수 있다. 이는 결함이 아니라 "즉시 취소"의 결과다.
- 취소되거나 끝나면 그 시점의 `MovementState` 를 다시 조회한다.
- 반복하지 않는다. Turn, Jumping 과 함께 매니페스트의 `loop` 가 false 다.

Landing 을 시작시키는 조건을 착지 충격(LandImpact)이라 부른다. 이동 상태가 아니라 한 순간의 사건이며, 플레이어 쪽에서 읽기 전용 신호로만 내보낸다.

- 접지가 아니었다가 접지로 바뀌는 순간에만 판정한다. 한 번의 접촉에서 한 번만 발생한다.
- 급강하 상태로 땅에 닿으면 조건 없이 발생한다.
- 그 밖에는 부딪히기 직전의 하강 속도가 `landImpactSpeed` 를 넘을 때만 발생한다. 과거에 얼마나 빨랐는지는 보지 않는다.
- 그래서 벽을 타고 내려오거나 대쉬로 감속한 뒤 닿으면 발생하지 않는다.
- 기준값은 평범한 점프의 착지 속도보다 충분히 높게 잡는다. 점프는 올라간 속도 그대로 돌아오므로 착지 속도가 `jumpSpeed` 와 같다. 기준이 그보다 낮으면 모든 점프가 착지 충격이 되어 버린다.

화면 흔들림이나 먼지 같은 효과는 아직 붙이지 않았다. 붙일 때도 같은 신호를 쓰면 된다.
...
6. Jump Start / Airborne / Land (2026-09-11 셋 다 적용 완료. Land 는 이동 상태가 아니라 착지 충격에 반응하는 Landing 으로 연결했다)
7. Air Dash (2026-09-11 적용 완료)
8. Hover / Fast Fall (2026-09-11 둘 다 적용 완료. Hover 는 코드의 SlowFalling 에 연결했다)
9. Wall / Hang / Slide (2026-09-11 코드의 WallSliding 에 적용 완료. Hang 과 Ground Slide 는 미착수)
10. Music
